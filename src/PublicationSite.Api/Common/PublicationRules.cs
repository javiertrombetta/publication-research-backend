...
public static class PublicationRules
{
    ...
    /// Supervisor has looked at a paper and after they have approved it — approval does not move
    /// it on, because the next step belongs to an administrator appointing a committee. Three
    /// places need to tell those two apart: the Supervisor's own queue, the administrator's list
    /// of papers ready for a committee, and the check that refuses to appoint one prematurely.
    /// Reading it from the status left the first two listing papers the third then rejected.
    ///
    /// It asks about the newest version specifically. A paper sent back and resubmitted carries
    /// the Supervisor's approval of the draft that was superseded, and that approval says nothing
    /// about the one now in front of them.
    /// </summary>
    /// <param name="approved">
