...
the short commit SHA (e.g. `:5708bec`), and this is what Render always runs.
...
Give it **Read & Write** scope. Copy it immediately, because Docker Hub will not show it again.
...
3. Service → **Environment**, fill in at minimum:
   ...
   | `Jwt__SigningKey` | `openssl rand -base64 64` , and do not reuse the dev key from `appsettings.Development.json` |
   | `Frontend__BaseUrl` | your frontend's URL (or `http://localhost:3000` until you have one) |
   | `Cors__AllowedOrigins__0` | same as above, and must match exactly for the frontend to be able to call this API |
   ...
   | `Seed__DemoData` | `"true"` on the shared instance the team tests against, so it comes up with an account for every role and a publication at every pipeline stage. **Remove it for production**, because every demonstration account shares one published password, and the default outside development is off |

4. **Manual Deploy → Deploy latest commit** (or just wait, since Render also polls the image tag
   ...
   . The service reports healthy straight away and the data appears over the following
